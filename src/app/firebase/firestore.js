...
export async function getComplaintsByUser(userId) {
  const snap = await getDocs(query(collection(db, "complaints"), where("userId", "==", userId)));
  return snap.docs
    .map((docItem) => ({ id: docItem.id, ...docItem.data() }))
    .sort((a, b) => (b.updatedAt?.seconds || 0) - (a.updatedAt?.seconds || 0));
}
...
export async function addComplaintMessage(complaintId, user, text, role) {
  await updateDoc(doc(db, "complaints", complaintId), {
    messages: arrayUnion({
      authorId: user.uid,
      authorName: user.displayName || user.email || (role === "admin" ? "Admin" : "User"),
      authorRole: role,
      text: text.trim(),
      createdAt: new Date().toISOString(),
    }),
    status: role === "admin" ? "in-progress" : "open",
    updatedAt: serverTimestamp(),
  });
}

export async function closeComplaint(complaintId) {
  await updateDoc(doc(db, "complaints", complaintId), {
    status: "closed",
...
export async function registerForSession(userId, sessionId) {
  ...
  if (sessionSnap.data().accessType === "paid") {
    throw new Error("Complete payment before registering for this session.");
  }
