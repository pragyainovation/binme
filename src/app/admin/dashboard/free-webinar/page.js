...
import { sendAdminNotification } from "../../../firebase/push";
...
export default function AdminFreeWebinarPage() {
  ...
  const notify = async (webinar) => {
    setNotificationId(webinar.id);
    try {
      const result = await sendAdminNotification(webinar.id, "webinar");
      window.alert(`Notification sent to ${result.sent} device${result.sent === 1 ? "" : "s"}.`);
    } catch (notifyError) {
      window.alert(notifyError.message || "Unable to send notification.");
    } finally {
    ...
    { header: "Registrations", accessorKey: "registrationCount", cell: ({ row }) => row.original.registrationCount || 0 },
    {
      ...
      cell: ({ row }) => {
        ...
        return <div style={styles.rowActions}>
          <button type="button" style={styles.viewButton} onClick={() => setSelectedWebinarId(selectedWebinarId === webinar.id ? null : webinar.id)} aria-label={selectedWebinarId === webinar.id ? "Hide registrations" : "View registrations"} title={selectedWebinarId === webinar.id ? "Hide registrations" : "View registrations"}>&#128065;</button>
          ...
          <button type="button" style={styles.notificationButton} onClick={() => notify(webinar)} disabled={notificationId === webinar.id} aria-label="Send Notification" title="Send Notification">&#128276;</button>
          <button type="button" style={styles.smallButton} onClick={() => editWebinar(webinar)} aria-label="Edit" title="Edit">&#9998;</button>
          <button type="button" style={styles.smallButton} onClick={() => toggleStatus(webinar)} aria-label={webinar.status === "inactive" ? "Activate" : "Deactivate"} title={webinar.status === "inactive" ? "Activate" : "Deactivate"}>{webinar.status === "inactive" ? "\u25b6" : "\u23f9"}</button>
        </div>;
      },
    },
  ];
...
const styles = {
  ...
  notificationButton: { border: 0, borderRadius: 8, padding: "8px 11px", background: "#d9f4f1", color: "#12665e", fontWeight: 700, cursor: "pointer" },
