...
export default function AdminComplaintDetailPage({ params }) {
  ...
  return <main className="complaint-page"><article className="complaint-detail-card"><header className="complaint-detail-header"><div><p>{complaint.userName} · {complaint.userEmail}</p><h1>{complaint.subject}</h1></div><span className={`complaint-status ${complaint.status}`}>{complaint.status}</span></header><p className="complaint-description">{complaint.description}</p><section className="complaint-conversation">{(complaint.messages || []).map((message, index) => <div key={`${message.createdAt}-${index}`} className={message.authorRole === "admin" ? "complaint-message own" : "complaint-message"}><strong>{message.authorName}</strong><p>{message.text}</p></div>)}</section>{complaint.status === "closed" ? <p className="complaint-empty">This complaint is closed.</p> : <form onSubmit={send} className="complaint-reply"><textarea value={text} onChange={(event) => setText(event.target.value)} placeholder="Reply to the user" required /><button type="submit" className="complaint-primary">Send Reply</button></form>}<button type="button" className="complaint-close" onClick={closeTicket} disabled={closing || complaint.status === "closed"}>{complaint.status === "closed" ? "Complaint Closed" : closing ? "Closing..." : "Close Complaint"}</button>{error ? <p className="complaint-error">{error}</p> : null}</article></main>;
}
