...
import { getAdminCourses, getCourseEnrollmentCount } from "@/features";
...
export default function AdminCoursesPage() {
  ...
  const load = async () => { const items = await getAdminCourses(); const coursesWithEnrollmentCounts = await Promise.all(items.map(async (course) => ({ ...course, enrolledCount: await getCourseEnrollmentCount(course.id) }))); setCourses(coursesWithEnrollmentCounts); setLoading(false); };
  useEffect(() => { load(); }, []);
...
const styles = { page:{minHeight:"100vh",background:"#f5f1ea",padding:"32px 20px"},container:{maxWidth:1100,margin:"0 auto"},header:{display:"flex",justifyContent:"space-between",alignItems:"center",gap:16,marginBottom:24},eyebrow:{margin:0,color:"#ff724f",fontWeight:800,textTransform:"uppercase"},title:{fontSize:36,margin:"6px 0 0"},primary:{background:"#16211f",color:"#fff",padding:"12px 18px",borderRadius:10,fontWeight:700},actions:{display:"flex",gap:8,flexWrap:"wrap"},enrollments:{padding:"8px 10px",background:"#e4f5d2",color:"#285b16",borderRadius:8,fontWeight:700},edit:{padding:"8px 10px",background:"#eee9ff",color:"#5a3f9a",borderRadius:8,fontWeight:700},lessons:{padding:"8px 10px",background:"#dfe8ff",color:"#2941a8",borderRadius:8,fontWeight:700},delete:{padding:"8px 10px",border:0,background:"#f7d9d9",color:"#8b2d2d",borderRadius:8,fontWeight:700} };
