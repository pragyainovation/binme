import { richTextToPlainText } from "@/features/policies/policy.content";

export function validateCourse(input) {
  if (!input?.title?.trim()) throw new Error("Course title is required.");
  const hasDescription = richTextToPlainText(input?.description);
  if (!hasDescription) throw new Error("Course description is required.");
