...
export default function ProfileSecurityTabs({ admin = false }) {
  ...
  return <main className="profile-page"><section className="profile-card">
    ...
    </section> : activeTab === "security" ? <section><p className="profile-tab-copy">Set a new password for your Firebase account.</p><PasswordChangeForm loginPath={admin ? "/admin/login" : "/login"} /></section> : admin && activeTab === "policies" ? <PolicyEditor /> : admin && activeTab === "social" ? <SocialLinksEditor /> : null}
  </section></main>;
}
